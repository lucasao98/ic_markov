...
class HeuristicM1PlusController extends Controller
{
    public function actionIndex()
    {
        ...
        if ($model->load($post) && $model->validate() && $model->periodo) {
            $start = \DateTime::createFromFormat('d/m/YH:i:s', $model->inicio . '24:00:00');
            $prevision_year = intval($start->format('Y'));

            while ($prevision_year >= 2011) {
                foreach ($transitions_matrix_fixed as $key => $value) {
                    ...
                    $file_transition_matrix_size = fopen("transition_matrix_size.txt", "a");
                    if (($model->metric == 'year') && (($ano_previsao - $model->periodo) < 2011)) {
                ...
                $model->inicio = $start->format('d/m/Y');
                $model->final = $final->format('d/m/Y');
            }

            var_dump($obj_accuracy_rate);
            exit();
