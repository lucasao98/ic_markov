...
class AutomaticHeuristicM3Controller extends Controller
{
    public function actionIndex()
    {
        ...
        if ($model->load($post) && $model->validate() && $model->periodo) {
            ...
            foreach ($model->actions as $action_name) {
                $start = $model->inicio;
                $final = $model->final;
                $consultas = 0;
                $acertou = 0;
                ...
                $action_info = [
                    'name' => $action_name,
                    'forecast_year_initial' => $model->inicio,
                    'forecast_year_final' => $model->final,
                    'hit_rate_percentage_three_states' => 0,
                    'hit_rate_percentage_inflection_dots' => 0,
                    'hit_rate_percentage_constant_variation' => 0,
                    'investment_strategy_1' => 0,
                    'investment_strategy_2' => 0,
                    'investment_strategy_3' => 0,
                    'observations_to_inflection_dots' => $model->qtde_obs,
                    'observations_to_constant_variation' => $model->qtde_up_down_constants,
                    'metric' => $model->metric,
                ...
                $action_info['hit_rate_percentage_inflection_dots'] = round($percentage_heuristica,2);
                $action_info['hit_rate_percentage_three_states'] = round(($t_acertou / $consultas) * 100, 2);
                $action_info['hit_rate_percentage_constant_variation'] = round($percentage_heuristica_m3, 2);
                $action_info['investment_strategy_1'] = $cliente_teste_1->getCash();
                $action_info['investment_strategy_2'] = $cliente_teste_2->getCash();
                $action_info['investment_strategy_3'] = $cliente_teste_3->getCash();

                array_push($actions_info, $action_info);
            }

            $session->set('retorno',$actions_info);
           
            $response->format = \yii\web\Response::FORMAT_JSON;
            return [
                'message' => json_encode($actions_info),
                'code' => 200,
            ];
        } else {
    ...
    public function actionResult()
    {
        ...
        $result_actions = $session->get('retorno');

        return $this->render('result',[
            'retorno_acoes' => $result_actions
        ]);
    }
}
